...
import type { CliAdapter, CliCompactPlan } from './types';
...
function protocolError(message: JsonMessage): string | undefined {
  ...
  return typeof message.error.message === 'string' ? message.error.message : '原生上下文整理失败';
}

/**
 * 执行 Claude Code 的上下文整理（Compact）操作。
 * @param plan 压缩计划对象
 * @param options 上下文整理选项
 * @returns 上下文整理结果
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    lines.on('line', (line) => {
      ...
      if (message.type === 'result' && message.is_error === true) {
        const result =
          typeof message.result === 'string' ? message.result : 'Claude Code 原生上下文整理失败';
...
/**
 * 执行上下文整理（Compact）操作，根据不同的 CLI 适配器选择相应的执行方式。
 * @param options 上下文整理选项
 * @returns 上下文整理结果
 */
export function compactCliSession(
  ...
  const plan = options.adapter.buildCompactPlan(options.sessionId, options.instructions);
