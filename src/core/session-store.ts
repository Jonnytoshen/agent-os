...
/**
 * 迁移旧版会话数据，补充缺失的 botId 和 workspaceDir 字段。
 *
 * @param row 原始会话数据
 * @param legacyBotId 旧版会话的默认 botId
 * @param defaultWorkspaces 每个 botId 对应的默认工作目录
 * @returns 迁移后的会话数据和是否进行了迁移
 */
function migrateLegacySession(
  ...
  const botId = typeof candidate.botId === 'string' ? candidate.botId : legacyBotId;
  if (needsWorkspace) {
